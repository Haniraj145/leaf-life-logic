...
function DashboardPreview() {
  ...
  return (
    <Section eyebrow="Smart Dashboard" title="Live monitoring at a glance"
      subtitle="Temperature, water, pH, humidity, nutrients, pump & light — beautifully visualized with real-time data.">
      <div className="grid gap-6 lg:grid-cols-[1.1fr_1fr]">
        <FadeIn>
          <Card className="bg-gradient-deep p-8 text-primary-foreground shadow-glow">
            <div className="mb-3 flex items-center gap-2 text-xs text-white/50"><span className="h-2 w-2 animate-pulse rounded-full bg-accent" /> Live from sensors</div>
            <div className="grid grid-cols-2 gap-6 md:grid-cols-3">
              <CircularGauge value={Math.round((sensor.ph_level-5.5)/1.5*100)} display={sensor.ph_level.toFixed(1)} label="pH Level" unit="optimal" color="oklch(0.75 0.18 200)" />
              <CircularGauge value={Math.round((sensor.ec_level-1.2)/1.2*100)} display={sensor.ec_level.toFixed(1)} label="EC Level" unit="mS/cm" color="oklch(0.78 0.20 145)" />
              <CircularGauge value={Math.round((sensor.temperature-18)/17*100)} display={`${sensor.temperature.toFixed(1)}°`} label="Temperature" unit="ideal" color="oklch(0.72 0.18 60)" />
              <CircularGauge value={sensor.humidity} display={`${Math.round(sensor.humidity)}%`} label="Humidity" unit="balanced" color="oklch(0.70 0.15 230)" />
              <CircularGauge value={sensor.water_level} display={`${Math.round(sensor.water_level)}%`} label="Water Tank" unit={sensor.water_level > 70 ? "full" : "refill"} color="oklch(0.65 0.18 230)" />
              <CircularGauge value={sensor.light_intensity} display={`${Math.round(sensor.light_intensity)}%`} label="Light" unit="grow mode" color="oklch(0.78 0.20 145)" />
            </div>
            <div className="mt-8 flex flex-wrap items-center gap-3 border-t border-white/10 pt-6 text-sm">
              <StatusPill on={sensor.pump_status} label={sensor.pump_status ? "Pump active" : "Pump off"} />
              <StatusPill on={sensor.light_status} label={`Grow light · ${Math.round(sensor.light_intensity)}%`} />
              <StatusPill label={`NPK: ${sensor.nutrient_n}/${sensor.nutrient_p}/${sensor.nutrient_k}`} />
